...
const TeacherReport = ({ userName, userEmail }) => {
  ...
  const fetchAttendanceDates = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/attendances/dates/${userEmail}`);
  ...
  const fetchClassCodes = async (attendanceDate) => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/attendances/clazzcode/${userEmail}/${attendanceDate}`);
  ...
  const fetchStudents = async (attendanceDate, clazzCode) => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/attendances/students/${userEmail}/${attendanceDate}/${clazzCode}`);
