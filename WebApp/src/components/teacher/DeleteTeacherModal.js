...
const DeleteTeacherModal = ({ fetchTeachers, visible, teacher, onCancel, onDeleteSuccess }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(`http://127.0.0.1:8000/api/teachers/${teacher.id}`); // Điều chỉnh endpoint API tương ứng
  ...
  return (
    ...
    >
      <p>Bạn có chắc chắn muốn xóa giáo viên?</p>
    </Modal>
  );
};
