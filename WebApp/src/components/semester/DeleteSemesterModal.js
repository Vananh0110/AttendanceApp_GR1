...
const DeleteSemesterModal = ({ fetchSemesters, visible, semester, onCancel, onDeleteSuccess }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(`http://127.0.0.1:8000/api/semesters/${semester.id}`); // Điều chỉnh endpoint API tương ứng
  ...
  return (
    ...
    >
      <p>Bạn có chắc chắn muốn xóa kỳ học?</p>
    </Modal>
  );
};
