...
const DeleteCourseModal = ({ fetchCourses, visible, course, onCancel, onDeleteSuccess }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(`http://127.0.0.1:8000/api/courses/${course.id}`); // Điều chỉnh endpoint API tương ứng
  ...
  return (
    ...
    >
      <p>Bạn có chắc chắn muốn xóa học phần?</p>
    </Modal>
  );
};
